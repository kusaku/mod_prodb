...
class Tracking(CallbackDelayer):
    ...
    def sendArenaResults(self, battleResults):
        ...
        for cid, player in self.players.iteritems():
            ...
            stats[cid] = dict()
            stats[cid][PlayerStats.DAMAGE_DONE] = ps.get('damageDealt', 0)
            stats[cid][PlayerStats.DAMAGE_BLOCKED] = ps.get('damageBlockedByArmor', 0)
            stats[cid][PlayerStats.DAMAGE_ASSIST] = ps.get('damageAssistedRadio', 0) + ps.get('damageAssistedStun', 0) + ps.get('damageAssistedTrack', 0)
