...
from .ProDBApi import getMatches, getPlayer, getMatchDetails, getTeamSquads, getMatchRoundsDetails
...
@functools.lru_cache()
def getMatchRoundKeyByPlayerCIDs_mock(*cids):
    time.sleep(random.random())
    result = str(uuid.uuid4())
    Logger.debug('[mock] getMatchRoundKeyByPlayerCIDs [{}] = {}'.format(','.join(cids), result))
    return result
...
async def getMatchRoundKeyByPlayerCIDs(team1_cids, team2_cids):
    ...
    if App().config.mockpoll:
        key = await run_in_executor(getMatchRoundKeyByPlayerCIDs_mock, *sorted(team1_cids + team2_cids))
    else:
...
@functools.lru_cache()
def getTeamSquadKeyByPlayerCIDs_mock(*cids):
    time.sleep(random.random())
    result = str(uuid.uuid4())
    Logger.debug('[mock] getTeamSquadKeyByPlayerCIDs [{}] = {}'.format(','.join(cids), result))
    return result
...
async def getTeamSquadKeyByPlayerCIDs(cids):
    from .App import App
    if App().config.mockpoll:
        key = await run_in_executor(getTeamSquadKeyByPlayerCIDs_mock, *sorted(cids))
    else:
...
@functools.lru_cache()
def getTeamSquadNameByPlayerCIDs_mock(*cids):
    time.sleep(random.random())
    result = str(uuid.uuid4())
    Logger.debug('[mock] getTeamSquadNameByPlayerCIDs [{}] = {}'.format(','.join(cids), result))
    return result
...
async def getTeamSquadNameByPlayerCIDs(cids):
    from .App import App
    if App().config.mockpoll:
        name = await run_in_executor(getTeamSquadNameByPlayerCIDs_mock, *sorted(cids))
    else:
